...
def test_token_for_default_dir_falls_back_to_keychain_on_macos(tmp_path, monkeypatch):
    ...
    # Isolate from a real long-lived-token env var/file on the host running the
    # tests (e.g. one set up via install-win.ps1's OAuth token phase) — this
    # test targets the file/Keychain fallback, not the long-lived override.
    with patch.object(mod, "_read_long_lived_token", return_value=None), \
         patch.object(mod, "_read_token_keychain", return_value="TOK_KEYCHAIN"):
...
def test_token_for_default_dir_long_lived_token_wins(tmp_path, monkeypatch):
    """The default dir's long-lived token (env var/installer file) overrides
    both the per-dir credentials file and Keychain — useful on a host with no
    interactive `claude` session to keep .credentials.json refreshed."""
    monkeypatch.setattr(mod, "DEFAULT_CONFIG_DIR", tmp_path)
    monkeypatch.setattr(mod.sys, "platform", "darwin")
    (tmp_path / ".credentials.json").write_text('{"accessToken":"TOK_FILE"}')
    with patch.object(mod, "_read_long_lived_token", return_value="TOK_LONGLIVED"), \
         patch.object(mod, "_read_token_keychain", return_value="TOK_KEYCHAIN"):
        assert read_token_for(tmp_path) == "TOK_LONGLIVED"


def test_token_for_non_default_dir_ignores_long_lived_token(tmp_path, monkeypatch):
    """A non-default (multi-plan) dir must NOT pick up the default dir's
    long-lived override — otherwise every configured plan would report the
    same account's usage."""
    monkeypatch.setattr(mod, "DEFAULT_CONFIG_DIR", Path("/not") / "this" / "dir")
    (tmp_path / ".credentials.json").write_text('{"claudeAiOauth":{"accessToken":"TOK_X"}}')
    with patch.object(mod, "_read_long_lived_token", return_value="TOK_LONGLIVED"):
        assert read_token_for(tmp_path) == "TOK_X"
